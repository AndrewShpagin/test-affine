...
`);
    ...
    const result=await page.evaluate(async f=>{
      ...
      const patch=frame=>{
        ...
        v.setUint16(20,frame-100,true);v.setUint16(22,f.width,true);v.setUint16(24,f.height,true);
      ...
      check(t.stats.keys===1,'key rendered for every region');
      ...
      check(t.state().stream===8&&t.state().frame===1,'retired stream resurrected');
