...
The loss is applied independently to each complete FlowX v3 UDP datagram after wrapping and immediately before `send()`. The sender reports deliberately dropped datagrams as `sim-loss`.
...
- `/flowx.html` should recover independently using the browser decoder;
- `/stream.mjpg` exercises the C++ decoder path in parallel;
